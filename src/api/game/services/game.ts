...
async function getGameInfo(slug) {
  try {
  // Transforma o slug do jogo para o formato do gog
  const gogSlug = slug.replaceAll("-", "_").toLowerCase();

  // Pega o corpo da pagina do jogo no gog
  const body = await axios.get(`https://www.gog.com/game/${gogSlug}`);
  // Cria um objeto DOM com o corpo da pagina
  const dom = new JSDOM(body.data);

  // Pegar a descrição do jogo da pagina do gog
  const raw_description = dom.window.document.querySelector(".description");

  // Pega a descrição do jogo em HTML
  const description = raw_description.innerHTML;
  // Pega uma pequena descrição do jogo em texto puro
  const short_description = raw_description.textContent.slice(0, 160);

  // Pega a classificação indicativa do jogo
  ...
  return {
    description,
    short_description,
    rating: ratingElement ? ratingElement : "BR0",
  };
 } catch (error) {
...
export default factories.createCoreService("api::game.game", () => ({
  async populate(params) {
    ...
    await createGames([products[3], products[1]]);
    } catch (error) {
